...
void InstrumentClusterStubImpl::setSpeedRpm(const std::shared_ptr<CommonAPI::ClientId> _client, uint16_t _sensorRpm, setSpeedRpmReply_t _reply)
{
    ...
    instrumentClusterCarInfo.setRpm(temp);

    temp = (uint16_t)((float)temp * 3.4);
    instrumentClusterCarInfo.setSpeed(temp);
...
{
    instrumentClusterCarInfo.setBattery(_battery);
...
{
    instrumentClusterCarInfo.setGear(_gear);
...
{
    instrumentClusterCarInfo.setDirection(_direction);
...
{
    instrumentClusterCarInfo.setLight(QString::fromStdString(_light));
